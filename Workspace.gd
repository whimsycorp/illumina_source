...
@export var CurrentCamera : PlayerCamera = PlayerCamera.new() ## The currently focused camera. Almost exclusively used client-side, never changes in most circumstances.
...
func _ready() -> void:
	...
	add_child(BasePart.new(Vector3(4, 4, 4), Vector3(0, 12, 0)))
	ServiceManager.LocalWorkspace = self
